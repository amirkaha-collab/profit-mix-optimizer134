...
# ── API key resolution (OpenAI) ───────────────────────────────────────────────

def _get_api_key() -> str:
    try:
        import streamlit as st
        ui_key = st.session_state.get("isa_api_key", "").strip()
        if ui_key:
            return ui_key
...
def _call_claude(prompt: str, system: str = "", max_tokens: int = 3200,
    ...
    """Call OpenAI Chat Completions API. Parameter names kept for compatibility."""
    api_key = _get_api_key()
    if not api_key:
        return "", "לא הוגדר מפתח OPENAI_API_KEY ב-Secrets."
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            return text if text else ("", "תגובה ריקה מהמודל.")
        elif resp.status_code == 401:
            return "", "מפתח API לא תקין — בדוק את OPENAI_API_KEY."
