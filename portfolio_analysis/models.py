...
def _infer_product_type(name: str) -> str:
    """
    Infer product type from fund/product name string.
    Returns one of the standard product_type values used across the app.
    """
    n = (name or "").strip()
    nl = n.lower()
    if "גמל להשקעה" in nl:
        return "גמל להשקעה"
    if "גמל" in nl or "קופת גמל" in nl:
        return "קופות גמל"
    if "פנסיה" in nl or "קרן פנסיה" in nl:
        return "קרנות פנסיה"
    if "השתלמות" in nl or "קרן השתלמות" in nl:
        return "קרנות השתלמות"
    if "פוליסה" in nl or "ביטוח מנהלים" in nl or "חיסכון" in nl:
        return "פוליסות חיסכון"
    return "קרנות השתלמות"  # default fallback
...
def import_from_session(st, df_long: pd.DataFrame, product_type: str) -> int:
    ...
    for r in raw:
        ...
        h = {
            "uid":              _new_uid(),
            "product_type":     r.get("product_type") or product_type,
